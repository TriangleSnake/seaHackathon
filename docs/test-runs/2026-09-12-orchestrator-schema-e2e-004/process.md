...
- Request ID: `TEST-ORC-SCHEMA-004`
- HTTP status: `200`
- Duration: `59622.75 ms`
...
2. `chat` called five tools through Agent Gateway, in this order:
   `get_evidence_records`, `find_conversation_accounts`,
   `get_account_activity` for the sender, `get_account_activity` for the recipient,
   and `get_virustotal_reputation` for the exact URL.
...
4. `chat` returned five investigated item scores and recommended checking the
...
8. `marketplace_info` used the validated evidence already in the ledger and returned
   four item scores. It found `RPT-0901`, whose report reason matched the off-platform
   verification behavior.
9. The three specialist aggregates were combined deterministically. The resulting
   fraud score was `0.52491199006`.
