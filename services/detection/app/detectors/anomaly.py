...
class AnomalyDetector:
    ...
    async def detect(self, context: DetectionContext) -> list[DetectionTrigger]:
        ...
        triggers.extend(self._login_diversity(context.evidence, context.as_of))
        triggers.extend(self._burst(context.evidence, "message", "sender_account_id", self.policy.messages_per_hour, "ANOMALY-CHAT-001", context.as_of))
        triggers.extend(self._burst(context.evidence, "product", "seller_account_id", self.policy.listings_per_hour, "ANOMALY-LISTING-001", context.as_of))
        triggers.extend(self._dispute_frequency(context.evidence, context.as_of))
        return triggers
    ...
    def _payment_churn(self, evidence: list[Evidence]) -> list[DetectionTrigger]:
        ...
        for transaction_id, items in groups.items():
            # This is a transaction-local sequence rule, so its window is anchored
            # to the transaction's latest attempt rather than the global clock.
            recent = _within_latest(items, timedelta(hours=1))
    ...
    def _login_diversity(self, evidence: list[Evidence], as_of: datetime | None) -> list[DetectionTrigger]:
        groups: dict[str, list[Evidence]] = defaultdict(list)
        for item in evidence:
