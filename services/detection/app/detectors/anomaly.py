...
def _within_latest(items: list[Evidence], window: timedelta) -> list[Evidence]:
    dated = [(item, _time(item)) for item in items]
    valid = [(item, timestamp) for item, timestamp in dated if timestamp is not None]
    if not valid:
        return items
    newest = max(timestamp for _, timestamp in valid)
    return [item for item, timestamp in valid if newest - timestamp <= window]
...
class AnomalyDetector:
    ...
    def _login_diversity(self, evidence: list[Evidence]) -> list[DetectionTrigger]:
        items = _within_latest(
            [item for item in evidence if item.type == "login_event" and item.data.get("success", True)],
            timedelta(hours=24),
        )
        ...
        return [
            DetectionTrigger(
                ...
                raw_result={"country_count": len(countries), "device_count": len(devices)},
    ...
    def _dispute_frequency(self, evidence: list[Evidence]) -> list[DetectionTrigger]:
        ...
        for account_id, items in groups.items():
            recent = _within_latest(items, timedelta(days=7))
