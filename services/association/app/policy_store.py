...
async def list_policies(agent: str, strategy: str | None = None) -> list[dict[str, Any]]:
    query = "SELECT agent,strategy,version,document,active,source,created_at FROM agent_policies WHERE agent=%s"
    params: tuple[Any, ...] = (agent,)
    if strategy:
        query += " AND strategy=%s"; params += (strategy,)
...
async def publish(agent: str, strategy: str, version: str, document: dict[str, Any], source: str) -> bool:
    async with await psycopg.AsyncConnection.connect(DATABASE_URL) as conn:
        async with conn.transaction():
            ...
            if existing:
                await conn.execute("UPDATE agent_policies SET active=true WHERE agent=%s AND strategy=%s AND version=%s", (agent,strategy,version))
            else:
                await conn.execute("""INSERT INTO agent_policies(agent,strategy,version,document,active,source)
                  VALUES (%s,%s,%s,%s::jsonb,true,%s)""", (agent,strategy,version,json.dumps(document),source))
            return True


async def save_draft(agent: str, strategy: str, version: str, document: dict[str, Any], source: str) -> bool:
    async with await psycopg.AsyncConnection.connect(DATABASE_URL) as conn:
        result = await conn.execute("""INSERT INTO agent_policies(agent,strategy,version,document,active,source)
          VALUES (%s,%s,%s,%s::jsonb,false,%s) ON CONFLICT(agent,strategy,version) DO NOTHING""",
          (agent,strategy,version,json.dumps(document),source))
