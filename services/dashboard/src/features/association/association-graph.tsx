...
import type { AssociationEntity, AssociationGraphResponse } from "@/types/association";
...
export function AssociationGraph({ graph = associationGraph }: { graph?: AssociationGraphResponse }) {
  const [selection, setSelection] = useState<Selection>({ kind: "entity", id: graph.focusEntityId });
  ...
  const nodes = useMemo<GraphNode[]>(() => {
    ...
    return graph.entities.map((entity) => {
      const isFocus = entity.id === graph.focusEntityId;
  ...
  }, [graph, query, selection]);

  const edges = useMemo<Edge[]>(() => graph.relations
    ...
    }), [graph, selection, threshold]);

  const selectedEntity = selection.kind === "entity" ? graph.entities.find((entity) => entity.id === selection.id) : undefined;
  const selectedRelation = selection.kind === "relation" ? graph.relations.find((relation) => relation.id === selection.id) : undefined;
  ...
  const directRelations = selectedEntity ? graph.relations.filter((relation) => relation.source === selectedEntity.id || relation.target === selectedEntity.id).length : undefined;
  const uniqueCaseCount = new Set(graph.entities.flatMap((entity) => entity.caseIds)).size;

  return <div className="association-layout">
    <section className="panel graph-panel">
      <header className="association-graph-head"><div><span className="association-graph-icon"><Network size={19}/></span><div><small>ASSOCIATION MAP</small><h2>案件關聯圖</h2></div></div><div className="graph-head-actions"><div className="graph-run-context"><span>{graph.strategy}</span><code>{graph.policyRef.version}</code></div><Link href="/policies/association" className="association-policy-link"><Settings2 size={15}/>Policy</Link></div></header>

      <div className="association-summary"><div><strong>{graph.entities.length}</strong><span>實體</span></div><div><strong>{graph.relations.length}</strong><span>關聯</span></div><div><strong>{uniqueCaseCount}</strong><span>相關案件</span></div><div><strong>{Math.round(Math.max(0, ...graph.relations.map((relation) => relation.confidence ?? 0)) * 100)}%</strong><span>最高信心度</span></div></div>
      ...
      <div className="graph-canvas">
        ...
        <div className="graph-legend"><span><i className="focus"/>核心實體</span><span><i className="related"/>關聯實體</span><span><GitBranch size={14}/>線條可點選</span><b>{edges.length} / {graph.relations.length} 條顯示中</b></div>
      </div>
    </section>
