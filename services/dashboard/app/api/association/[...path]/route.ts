...
function isAllowed(method: string, path: string) {
  return method === "GET" && (path === "health" || path === "association/jobs" || /^association\/jobs\/[A-Za-z0-9][A-Za-z0-9._-]*$/.test(path));
}
...
async function proxy(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  try {
    const upstream = await fetch(`${baseUrl()}/${path}${request.nextUrl.search}`, { cache: "no-store", signal: AbortSignal.timeout(8_000) });
