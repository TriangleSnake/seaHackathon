...
import { AgentModelControl } from "@/features/system/agent-model-control";
import { systemServices } from "@/lib/api/operations";
import { LiveSystemStatus } from "@/features/system/live-system-status";

export default function SystemPage() {
  if (process.env.DASHBOARD_DATA_MODE !== "demo") return <AppShell><div className="page-head"><div><div className="breadcrumb">Infrastructure / Live</div><h1>系統控制台</h1><p>直接探測已部署元件，並控制 Agent 後續工作使用的模型。</p></div></div><LiveSystemStatus/><AgentModelControl/></AppShell>;
  return <AppShell>
    <div className="page-head"><div><div className="breadcrumb">Infrastructure / Live</div><h1>系統控制台</h1><p>檢視服務健康度，並控制每個 Agent 後續任務使用的模型。</p></div><span className="workspace-badge healthy"><i/>系統運作中</span></div>
